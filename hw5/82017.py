...
def probClass(classOfEl, allData, lmbd):  # P(Class = classOfEl)
    return math.log2((totalLenClass(classOfEl, allData) + lmbd)/(len(allData) + 2*lmbd))
...
def condProb(training_set, learning_set, classOfEl, i, lmbd):  # P(x1=aj/value/|class=classOfEl)
    ...
    return math.log2((countSection + lmbd)/(totalLenClass(classOfEl, learning_set) + (len(training_set)-1)*lmbd))
...
def split10fold(allSet):
    saver = list()
    count = math.floor(len(allSet)/10)  # 43
    for i in range(10):
...
def acurrancy(lmbd):
    ...
    for i in range(10):
        ...
        for i in range(len(training_set)):
            res = res + (resultClasses[i] == currentClassList[i]) # count of equal
        acc += res/len(training_set)
    return (acc/10)*100


print("Accurancy: " + str(acurrancy(1)))
